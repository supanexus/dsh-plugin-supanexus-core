...
import { matchesStoredKeyPrefix } from "../../shared/key-prefix.js";
    ...
    ctx.effect(() => ctx.connection.fetch.register({
        ...
        fetch: async (request) => {
            try {
                ...
                if (!aligned) {
                    return jsonResponse(apiErr('API Key 已变更，请打开设置 → 模型，重新执行 SupaNexus 快速配置。', WALLET_ERROR.credentialMismatch), 400);
                }
