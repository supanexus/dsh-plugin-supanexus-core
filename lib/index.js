...
async function resolveAuthLine(ctx, config, options = {}) {
	...
	if (fromConfig.length > 0) {
		...
		if (line === void 0) throw new Error(`未知线路：${fromConfig}`);
		await patchSettings(ctx, {
			resolvedLine: line.id,
			resolvedAt: now,
			latencyMs: 0
	...
	if (fromUser.length > 0) {
		...
		if (line === void 0) throw new Error(`未知线路：${fromUser}`);
		await patchSettings(ctx, {
			resolvedLine: line.id,
			resolvedAt: now,
			latencyMs: 0
...
*/
async function resolveEnvLine(_ctx, config, options = {}) {
	const now = Date.now();
	if (!options.force && envCached !== void 0 && now < envCacheExpiresAt) {
		const hit = envCached;
		return {
			winner: hit,
...
function isAuthLineLocked(config) {
...
//#endregion
//#region lib/types/shared/reasoning-efforts.js
...
function modelNeedsReasoningEfforts(modelID) {
...
async function listModels(line, apiKey) {
	...
	return models.map((model) => {
		...
		const reasoning = modelNeedsReasoningEfforts(model.id) || supported.includes("reasoning");
		return {
			id: model.id,
			...model.name === void 0 ? {} : { name: model.name },
			...model.context_length !== void 0 && model.context_length > 0 ? { contextWindow: model.context_length } : {},
...
function registerAuthRoutes(ctx, config) {
	...
	ctx.effect(() => ctx.connection.fetch.register({
		...
		fetch: async (request) => {
			try {
				...
				return jsonResponse$3(apiErr(error instanceof Error ? error.message : "无法启动授权。"), 400);
			}
		}
	...
	ctx.effect(() => ctx.connection.fetch.register({
		...
		fetch: async (request) => {
			...
			if (flow === void 0) return jsonResponse$3(apiErr("授权流程不存在或已过期。"), 404);
			return jsonResponse$3(apiOk(flowStatus(flow)));
		}
...
function toLineStatusRows(lines) {
...
//#endregion
//#region lib/types/host/line/routes.js
...
function registerLineRoutes(ctx, config) {
	ctx.effect(() => ctx.connection.fetch.register({
		...
		fetch: async () => {
			try {
				...
				return jsonResponse$2(apiErr(error instanceof Error ? error.message : "无法解析访问区域。"), 400);
			}
		}
...
function buildInstallSpecFor(repo, installCode) {
	...
	try {
		...
		if (u.hostname === "gitee.com" || u.hostname === "www.gitee.com") {
			const base = `git+https://gitee.com/${u.pathname.replace(/^\/+/, "").replace(/\.git$/, "")}.git`;
			const ref = repo.default_ref?.trim();
			return ref !== void 0 && ref.length > 0 ? `${base}#${ref}` : base;
		}
	} catch {}
	return installCode;
}
...
function readInstalledPackageVersion(profileName, packageName) {
	const packageJsonPath = join(packageInstallDir(resolveProfileDir(profileName), packageName), "package.json");
...
//#endregion
//#region lib/types/shared/key-prefix.js
...
*/
function matchesStoredKeyPrefix(secret, keyPrefix) {
...
async function resolveApiKeySecret(ctx) {
	const value = (await ctx.credentials.resolve(credentialRef(CREDENTIAL_REF)))?.value?.trim();
...
async function isCredentialAligned(ctx, keyPrefix) {
...
async function ensureAccessToken(ctx, config, locale) {
	...
	const now = Date.now();
	if (accessCache !== void 0 && accessCache.expiresAt > now + 15e3 && accessCache.lineId === line.id) return {
		...
		fetch: async () => {
			try {
				...
				return jsonResponse(apiOk({
					connected,
					credentialAligned: connected ? await isCredentialAligned(ctx, settings.keyPrefix) : true,
					usagePoliciesUrl: usagePoliciesUrlFor(config, await pickLineForConsole(ctx, config)),
	...
	ctx.effect(() => ctx.connection.fetch.register({
		...
		fetch: async (request) => {
			try {
				...
				const settings = readSettings(ctx);
				if (!await isCredentialAligned(ctx, settings.keyPrefix)) return jsonResponse(apiErr("API Key 已变更，请打开设置 → 模型，重新执行 SupaNexus 快速配置。", WALLET_ERROR.credentialMismatch), 400);
