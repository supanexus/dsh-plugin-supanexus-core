...
import { resolveProfileDir } from './profile-deps.ts'
...
export function readUserPatchState(patchPath: string): PatchState {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (/^ {2}disabled: true\s*$/u.test(next)) {
      ...
      if (rowId !== undefined) disables.push(rowId)
    } else if (/^ {2}disabled: false\s*$/u.test(next)) {
      const rowId = disableRow[1]
      if (rowId !== undefined) forced.push(rowId)
    }
  }
  return { disables, forced }
}
...
export function bundleInsertIds(profileDir: string, packageName: string): string[] {
...
export function packageEnabled(
  profileName: string,
  packageName: string,
): { enabled: boolean; rowIds: readonly string[] } {
  const profileDir = resolveProfileDir(profileName)
  const patchPath = join(profileDir, 'cordis.patch.yml')
  const rowIds = bundleInsertIds(profileDir, packageName)
  if (rowIds.length === 0) return { enabled: true, rowIds }
  const state = readUserPatchState(patchPath)
...
export async function setPackageEnabled(
  ...
  packageName: string,
  enabled: boolean,
): Promise<{ ok: boolean; reason: string | null; rowIds: readonly string[] }> {
  const profileDir = resolveProfileDir(profileName)
  const patchPath = join(profileDir, 'cordis.patch.yml')
  const rowIds = bundleInsertIds(profileDir, packageName)
  if (rowIds.length === 0) {
  ...
  let last: { ok: boolean; reason: string | null } = { ok: true, reason: null }
