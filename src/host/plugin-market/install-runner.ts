...
export function resolveCliEntry(config: Config): string {
  ...
  throw new Error(
    'DSH CLI 未找到。请设置 DSH_CLI_ENTRY 或先在 engine 目录执行 pnpm run build。',
  )
}
...
export function runProfilePluginInstall(
  config: Config,
  spec: string,
): InstallRunResult {
...
export function runProfilePluginRemove(
  config: Config,
  packageName: string,
): InstallRunResult {
...
function runProfilePluginCommand(
  config: Config,
