...
import { runProfilePluginInstall, runProfilePluginRemove } from './install-runner.ts'
import { packageEnabled, setPackageEnabled } from './patch-layer.ts'
import { isDirectLoopbackRequest, scheduleHostRelaunch } from './restart-host.ts'
import { buildUpgradeStatusEntry } from './upgrade-status.ts'
...
function formatInstallError(run: { stderr: string; exitCode: number | null }): string {
...
function tryInstallWithFallback(
  config: Config,
  listing: { package_name: string; install_code: string },
  candidates: readonly PluginListingRepository[],
): { success: PluginInstallSuccess } | { failure: string; attempts: PluginInstallAttempt[] } {
  ...
  for (const repo of candidates) {
    ...
    if (run.ok) {
      const successAttempts: PluginInstallAttempt[] = [
        ...attempts,
        { ...attemptBase, ok: true },
      ]
      const success: PluginInstallSuccess = {
        ok: true,
        spec,
...
export function registerPluginMarketRoutes(ctx: Context, config: Config): void {
    ...
    fetch: async (request) => {
      try {
        ...
        if (action === 'restart') {
          ...
          return jsonResponse(apiOk({ restarting: true as const }), 202)
        }
  ...
  ctx.effect(() => ctx.connection.fetch.register({
    ...
    fetch: async (request) => {
      try {
        ...
        const data: PluginUpgradeStatusData = { items: resolved }
        return jsonResponse(apiOk(data))
      } catch (error: unknown) {
  ...
  ctx.effect(() => ctx.connection.fetch.register({
    ...
    fetch: async (request) => {
      try {
        ...
        const data: PluginRemoteVersionData = {
  ...
  ctx.effect(() => ctx.connection.fetch.register({
    ...
    fetch: async (request) => {
      try {
        ...
        const data: PluginToggleStatusData = { items }
        return jsonResponse(apiOk(data))
      } catch (error: unknown) {
