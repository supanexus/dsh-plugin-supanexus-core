...
import { dataPlaneBase } from '../../shared/line.ts'
import {
  CREDENTIAL_REF,
...
import { exchangeToken, listModels } from './harness-client.ts'
import { acquireDeviceSecret } from './acquire-device-secret.ts'
import { renderErrorPage, renderSuccessPage } from './callback-page.ts'
import { patchSettings } from '../settings/device.ts'
...
export async function handleOAuthCallback(
  ...
  try {
    ...
    const { secret, credential } = await acquireDeviceSecret(
      ctx,
      config,
      flow.line,
      tokens.accessToken,
