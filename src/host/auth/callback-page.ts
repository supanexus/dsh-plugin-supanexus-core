...
const COPY: Record<'zh-CN' | 'en-US', Record<CallbackPageKind, CallbackCopy>> = {
  'zh-CN': {
    success: {
      title: '授权成功',
...
export function callbackLocale(locale: string | undefined): 'zh-CN' | 'en-US' {
...
export function renderSuccessPage(locale?: string): string {
  return pageShell('success', callbackLocale(locale), '', 3)
}

/** Render the OAuth error page with a safe, escaped detail message. */
export function renderErrorPage(message: string, locale?: string): string {
