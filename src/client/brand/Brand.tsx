 ...
 */
export function SupaNexusBrandMark({ size, className }: SupaNexusBrandMarkProps) {
  return (
    <svg
      viewBox="0 0 512 512"
      width={size}
 ...
 */
export function SupaNexusBrandName({ locale }: SupaNexusBrandNameProps) {
  const t = brandT(locale)
  return (
    <div className={css.brandNameStack}>
      <span className={css.wordmark} aria-label="SupaNexus">
        SupaNexus
      </span>
...
export function SupaNexusBrandNameSlot({ ctx }: { readonly ctx: ClientContext }) {
  const locale = useClientLocale(ctx)
  return <SupaNexusBrandName locale={locale} />
}
