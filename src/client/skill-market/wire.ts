...
import {
  ...
  PLUGIN_UNINSTALL_PATH,
  PLUGIN_UPGRADE_STATUS_PATH,
  PLUGIN_REMOTE_VERSION_PATH,
  PLUGIN_TOGGLE_STATUS_PATH,
  PLUGIN_TOGGLE_PATH,
  RESTART_HOST_PATH,
  formatInstallAttemptsLog,
  type PluginInstallAttempt,
  type PluginInstallFailure,
  type PluginInstallResponse,
  type PluginCategoriesListData,
  type PluginListingsListData,
  type PluginUninstallFailure,
  type PluginUninstallResponse,
  type PluginUpgradeStatusData,
  type PluginRemoteVersionData,
  type PluginToggleResult,
  type PluginToggleStatusData,
...
type ListBody = PluginListingsListData & OkBody
type CategoriesBody = PluginCategoriesListData & OkBody
type InstallSuccessBody = Extract<PluginInstallResponse, { ok: true }> & OkBody
type InstallApiBody = (InstallSuccessBody | PluginInstallFailure) & {
  readonly log?: string
  readonly attempts?: readonly PluginInstallAttempt[]
}
type UninstallSuccessBody = Extract<PluginUninstallResponse, { ok: true }> & OkBody
type UninstallApiBody = (UninstallSuccessBody | PluginUninstallFailure) & { readonly log?: string }
...
export async function listPluginListings(
  params?: ListPluginListingsParams,
  ...
  const body = await parseJson<ListBody>(response)
  return {
    items: body.items ?? [],
...
export async function listPluginCategories(
  ...
  if (params?.locale !== undefined) url.searchParams.set('locale', params.locale)
  const response = await fetch(url)
  const body = await parseJson<CategoriesBody>(response)
...
export async function installPlugin(
  installCode: string,
  locale?: string,
): Promise<Extract<PluginInstallResponse, { ok: true }> & { readonly log?: string }> {
  ...
  const response = await fetch(url)
  const body = await readResponseBody(response) as InstallApiBody
  ...
  const log = body.attempts !== undefined
    ...
    : undefined
  const success = body as InstallSuccessBody
...
export async function uninstallPlugin(
  packageName: string,
  installCode?: string,
): Promise<Extract<PluginUninstallResponse, { ok: true }> & { readonly log?: string }> {
  ...
  const response = await fetch(url)
  const body = await readResponseBody(response) as UninstallApiBody
  const failureMessage = 'message' in body && typeof body.message === 'string' && body.message.length > 0
    ? body.message
    : undefined
  if (!response.ok || body.ok === false) {
  ...
  const log = typeof body.log === 'string' && body.log.length > 0
    ? body.log
    : undefined
  const success = body as UninstallSuccessBody
  return log !== undefined ? { ...success, log } : success
}

/** Batch upgrade hints for installed plugins via Host bridge. */
export async function fetchPluginUpgradeStatus(
  installCodes: readonly string[],
  locale?: string,
): Promise<PluginUpgradeStatusData> {
  const url = new URL(PLUGIN_UPGRADE_STATUS_PATH, window.location.origin)
  url.searchParams.set('installCodes', installCodes.join(','))
  if (locale !== undefined && locale.length > 0) url.searchParams.set('locale', locale)
  const response = await fetch(url)
  const body = await parseJson<PluginUpgradeStatusData & OkBody>(response)
  return { items: body.items ?? [] }
}

/** Fetch remote package.json version for upgrade confirm. */
export async function fetchPluginRemoteVersion(
  installCode: string,
  locale?: string,
): Promise<PluginRemoteVersionData> {
  const url = new URL(PLUGIN_REMOTE_VERSION_PATH, window.location.origin)
  url.searchParams.set('installCode', installCode)
  if (locale !== undefined && locale.length > 0) url.searchParams.set('locale', locale)
  const response = await fetch(url)
  const body = await parseJson<PluginRemoteVersionData & OkBody>(response)
  return {
...
export async function fetchPluginToggleStatus(
  packageNames: readonly string[],
): Promise<PluginToggleStatusData> {
  const url = new URL(PLUGIN_TOGGLE_STATUS_PATH, window.location.origin)
  url.searchParams.set('packageNames', packageNames.join(','))
  const response = await fetch(url)
  const body = await parseJson<PluginToggleStatusData & OkBody>(response)
  return { items: body.items ?? [] }
}
...
export async function setPluginEnabled(
  ...
  const response = await fetch(url)
  const body = await parseJson<PluginToggleResult & OkBody>(response)
  return {
...
export async function restartHostService(): Promise<void> {
  ...
  for (const url of candidates) {
    const response = await fetch(url)
    const body = await readResponseBody(response) as { ok?: boolean; message?: string }
