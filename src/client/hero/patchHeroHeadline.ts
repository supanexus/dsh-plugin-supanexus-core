...
const HEADLINE_REVERSALS = new Map(
  [...HEADLINE_REPLACEMENTS].map(([from, to]) => [to, from] as const),
)
 ...
 */
export function patchHeroHeadlines(root: ParentNode): void {
...
export function restoreHeroHeadlines(root: ParentNode): void {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT)
  let node = walker.nextNode()
  while (node !== null) {
    const text = node.textContent
 ...
 * @param root - DOM subtree to scan.
 */
export function hideHeroPreviewBadge(root: ParentNode): void {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT)
  let node = walker.nextNode()
  while (node !== null) {
...
export function showHeroPreviewBadge(root: ParentNode): void {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT)
  let node = walker.nextNode()
  while (node !== null) {
    const text = node.textContent?.trim()
...
export function patchHeroChrome(root: ParentNode): void {
  patchHeroHeadlines(root)
  hideHeroPreviewBadge(root)
}

/** Undo SupaNexus hero chrome patches under `root`. */
export function clearHeroChrome(root: ParentNode): void {
 ...
 */
export function watchHeroChromePatches(): () => void {
